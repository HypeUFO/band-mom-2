...
class EventDetails extends Component {
  ...
  componentWillMount() {
    ...
    database
      .ref("events")
      .child(this.props.match.params.eventId)
      .on("value", () => {
        Promise.resolve()
          .then(() => {
  ...
  render() {
    const { event, eventEdit, band, match } = this.props;
    ...
    let breadcrumbs =
      event && band
        ? [
            {
              link: `/${match.params.userId}/bands/${
                match.params.bandId
              }/dashboard`,
              name: band.name
            },
            ...
            {
              link: null,
              name: `${event.venue}: ${moment(event.date).format("MM/DD/YY")}`
            }
          ]
        : [
            {
    ...
    let classes = classNames("event-details__container", {
      "event-details__container--hidden": !!eventEdit
    });
    return (
      <div className="page__container">
        ...
        <Subheader
          ...
          buttonHide={!this.state.editable}
        ...
        <div className="page__content page__content--two-col">
          <div className={classes}>
            ...
            {this.renderForm()}
            <div id="bottom" style={{ width: "100%", height: 80 }} />
          </div>
        </div>
      </div>
    );
