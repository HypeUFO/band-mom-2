    ...
    return database
      ...
      .then(() => {
        ...
        Object.keys(groupMembers).map(key => {
          const newNotificationKey = database
            .ref()
            .child("notifications")
            .child(groupMembers[key])
            .push().key;
          database
            .ref()
            .child("userEvents")
          ...
          if (key !== user.id) {
            updates[
              `/notifications/${groupMembers[key]}/${newNotificationKey}`
            ] = notification;
          }
        });
        ...
        if (band.stageplots) {
          Object.keys(band.stageplots).map(key => {
            storage
              .ref()
              .child(`/stageplots/${band.id}/${band.stageplots[key].name}`)
              .delete();
          });
        }
        if (band.logoName) {
          storage
            .ref()
            .child(`/groupLogos/${band.id}/${band.logoName}`)
            .delete();
        }
...
export function updateMemberEdit() {
  return dispatch => {
    Promise.resolve()
      .then(() => dispatch(updateMemberEditRequestedAction()))
      .then(() => {
        dispatch(updateMemberEditFulfilledAction());
      })
      .catch(err => {
        dispatch(updateMemberEditRejectedAction(err));
      });
  };
}

function updateMemberEditRequestedAction() {
  return {
    type: ActionTypes.UPDATE_MEMBER_EDIT_REQUESTED
  };
}

function updateMemberEditFulfilledAction() {
  return {
    type: ActionTypes.UPDATE_MEMBER_EDIT_FULFILLED
  };
}

function updateMemberEditRejectedAction(error) {
  return {
    type: ActionTypes.UPDATE_MEMBER_EDIT_REJECTED,
    error
  };
}

export function updateMember(member, bandId, user) {
  return dispatch => {
    dispatch(updateMemberRequestedAction());

    const updates = {};

    const notification = {
      message: `${user.displayName || user.email} has edited ${
        member.displayName
      }'s role`
    };

    updates[`/groups/${bandId}/members/${member.id}`] = member;
    ...
    return database
      ...
      .update(updates)
      .then(() => {
        dispatch(updateMemberFulfilledAction(member));
      })
      .catch(error => {
        dispatch(updateMemberRejectedAction(error));
      });
  };
}

function updateMemberRequestedAction() {
  return {
    type: ActionTypes.UPDATE_MEMBER_REQUESTED
  };
}

function updateMemberFulfilledAction() {
  return {
    type: ActionTypes.UPDATE_MEMBER_FULFILLED
  };
}

function updateMemberRejectedAction(error) {
  return {
...
export function sendGroupInvite(band, recipient, user) {
  ...
  return dispatch => {
    ...
    Promise.resolve().then(() => {
      ...
      const notification = {
        ...
        message: `${user.displayName || user.email} has invited you to join ${
          band.name
        }`,
      ...
      updates[
        `/notifications/${recipientId}/${newNotificationKey}`
      ] = notification;
    ...
    return database
      ...
      .then(() => {
        ...
        return database
          ...
          .then(() => {
            const notification = {
              message: `${user.displayName || user.email} has joined ${
                band.name
              }`,
              band: band
            };
            ...
            updates["/groups/" + bandId + "/members/" + userId] = user;
...
export function leaveBand(band, user) {
  return dispatch => {
    ...
    if (groupEvents && Object.keys(groupEvents).length > 0) {
          ...
          Object.keys(snap.val()).map(key => {
            if (key !== user.id) {
              const newNotificationKey = database
                ...
                .push().key;
              return (updates[
                `/notifications/${key}/${newNotificationKey}`
              ] = notification);
            }
          });
        }
